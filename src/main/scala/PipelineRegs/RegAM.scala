...
class RegAM extends Module
{
    ...
    Array(
        ...
        io.wr_en_out,  io.str_en_out,   io.load_en_out, io.br_en_out,    io.jalr_en_out,
        io.jal_en_out, io.auipc_en_out, io.lui_en_out,  io.sb_en_out,    io.sh_en_out,
        io.sw_en_out,  io.lb_en_out,    io.lh_en_out,   io.lw_en_out,    io.lbu_en_out,
        io.lhu_en_out, io.PC4_out,
        
        // Registers
        PC,            alu,             rd_addr,        rs2_data,        imm,
        wr_en,         str_en,          load_en,        br_en,           jalr_en,
        jal_en,        auipc_en,        lui_en,         sb_en,           sh_en,
        sw_en,         lb_en,           lh_en,          lw_en,           lbu_en,
        lhu_en,        PC4
    ) zip Array(
        // Output pins
        PC_out,        alu_out,         rd_addr_out,    rs2_data_out,    imm_out,
        wr_en_out,     str_en_out,      load_en_out,    br_en_out,       jalr_en_out,
        jal_en_out,    auipc_en_out,    lui_en_out,     sb_en_out,       sh_en_out,
        sw_en_out,     lb_en_out,       lh_en_out,      lw_en_out,       lbu_en_out,
        lhu_en_out,    PC4_out,

        // Registers
        PC_in,         alu_in,          rd_addr_in,     rs2_data_in,     imm_in,
        wr_en_in,      str_en_in,       load_en_in,     br_en_in,        jalr_en_in,
        jal_en_in,     auipc_en_in,     lui_en_in,      sb_en_in,        sh_en_in,
        sw_en_in,      lb_en_in,        lh_en_in,       lw_en_in,        lbu_en_in,
        lhu_en_in,     PC4_in
    ) foreach
    {
        x => x._1 := x._2
